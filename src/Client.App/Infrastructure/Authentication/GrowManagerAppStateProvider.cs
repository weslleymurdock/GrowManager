...
{
    public class GrowManagerAppStateProvider : AuthenticationStateProvider
    {
        private readonly HttpClient _httpClient;
        private readonly IRepository<ClientPreference> _localStorage;

        public GrowManagerAppStateProvider(
        ...
        {
            var preference = await _localStorage.GetItemAsync<ClientPreference>(x => x.Preference == StorageConstants.Local.AuthToken);
            if (preference == null)
            {
                return new AuthenticationState(new ClaimsPrincipal(new ClaimsIdentity()));
            }
            if (string.IsNullOrWhiteSpace(preference.AuthToken))
            {
                return new AuthenticationState(new ClaimsPrincipal(new ClaimsIdentity()));
            }
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", preference.AuthToken);
            var state = new AuthenticationState(new ClaimsPrincipal(new ClaimsIdentity(GetClaimsFromJwt(preference.AuthToken), "jwt")));
